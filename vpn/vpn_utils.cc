...
DWORD RegisterSublayer(HANDLE engine_handle, GUID uuid) {
  ...
  if (FwpmSubLayerGetByKey0(engine_handle, &uuid, &sublayer_ptr) ==
      ERROR_SUCCESS) {
    LOG(INFO) << "Using existing sublayer";
    if (sublayer_ptr)
  ...
  if (result != (DWORD)FWP_E_ALREADY_EXISTS && result != ERROR_SUCCESS) {
    LOG(ERROR) << "Failed to add a persistent sublayer with "
                  "BRAVEVPN_DNS_SUBLAYER UUID";
    return result;
  }
  LOG(INFO) << "Added a persistent sublayer with BRAVEVPN_DNS_SUBLAYER UUID";
  return ERROR_SUCCESS;
}
...
DWORD PermitQueriesFromTAP(HANDLE engine_handle,
  ...
  if (!index) {
    LOG(ERROR) << "Failed to get index for adapter:" << connection_name;
  ...
  if (result) {
    LOG(ERROR) << "Convert interface index to luid failed:" << std::hex
               << result;
    return result;
  }
  ...
  if (result) {
    LOG(ERROR) << "Add filter to permit IPv4 DNS traffic through TAP failed:"
               << std::hex << result;
    return result;
  }
  ...
  result = FwpmFilterAdd0(engine_handle, &Filter, nullptr, &filterid);
  if (result) {
    LOG(ERROR) << "Add filter to permit IPv6 DNS traffic through TAP failed:"
               << std::hex << result;
  }
  return result;
}
...
DWORD AddDnsFilters(HANDLE engine_handle, const std::string& connection_name) {
  if (!engine_handle) {
    LOG(ERROR) << "Engine handle cannot be null";
    return ERROR_INVALID_PARAMETER;
  }
  auto result = RegisterSublayer(engine_handle, GetVpnDnsSublayerGUID());
  if (result) {
    LOG(ERROR) << "Open FWP session failed, error code:" << std::hex << result;
    return result;
  }
  ...
  if (result) {
    LOG(ERROR) << "Add filter to block IPv4 DNS traffic failed:" << std::hex
               << result;
    return result;
  }
  ...
  if (result) {
    LOG(ERROR) << "Add filter to block IPv6 DNS traffic failed:" << std::hex
               << result;
    return result;
  }
  ...
  if (result) {
    LOG(ERROR)
        << "Add filter to permit IPv4 and IPv6 DNS queries from TAP failed:"
        << std::hex << result;
    return result;
  }

  LOG(ERROR) << "Added block filters for all interfaces";
