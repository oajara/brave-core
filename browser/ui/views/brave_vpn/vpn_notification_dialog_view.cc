...
constexpr char kBraveVPNLearnMoreURL[] = "https://support.brave.com/";
...
void VpnNotificationDialogView::Show(Browser* browser) {
  ...
  constrained_window::CreateBrowserModalDialogViews(
      new VpnNotificationDialogView(browser),
...
VpnNotificationDialogView::VpnNotificationDialogView(Browser* browser)
  ...
  RegisterWindowClosingCallback(base::BindOnce(
      &VpnNotificationDialogView::OnClosing, base::Unretained(this)));
  SetAcceptCallback(base::BindOnce(&VpnNotificationDialogView::OnAccept,
  ...
  views::StyledLabel::RangeStyleInfo link_style =
      views::StyledLabel::RangeStyleInfo::CreateForLink(base::BindRepeating(
          &VpnNotificationDialogView::OnLearnMoreLinkClicked,
          base::Unretained(this)));
...
VpnNotificationDialogView::~VpnNotificationDialogView() = default;

void VpnNotificationDialogView::OnLearnMoreLinkClicked() {
  chrome::AddSelectedTabWithURL(browser_, GURL(kBraveVPNLearnMoreURL),
                                ui::PAGE_TRANSITION_AUTO_TOPLEVEL);
  AcceptDialog();
}

ui::ModalType VpnNotificationDialogView::GetModalType() const {
  return ui::MODAL_TYPE_WINDOW;
}

bool VpnNotificationDialogView::ShouldShowCloseButton() const {
  return false;
}

bool VpnNotificationDialogView::ShouldShowWindowTitle() const {
  return false;
}

void VpnNotificationDialogView::OnAccept() {
  close_window_ = true;
}

void VpnNotificationDialogView::OnClosing() {
  prefs_->SetBoolean(prefs::kBraveVPNShowNotificationDialog,
                     !dont_ask_again_checkbox_->GetChecked());
}

BEGIN_METADATA(VpnNotificationDialogView, views::DialogDelegateView)
END_METADATA
