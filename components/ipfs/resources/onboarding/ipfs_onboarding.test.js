...
test('Validate origin success', () => {
  expect(
    messageHandler({
      ...
      'data': { 'command': 'ipfs', 'code': 100, 'value': '' }
    })
  ).toStrictEqual(true)
})
...
test('No data payload', () => {
  expect(
    messageHandler({ 'type': 'message', 'origin': document.location.origin })
  ).toStrictEqual(false)
})
...
test('Code and value as numbers', () => {
  expect(
    messageHandler({
      'type': 'message',
      'origin': document.location.origin,
      'data': { 'command': 'ipfs', 'code': `100`, 'value': '1' }
    })
  ).toStrictEqual(true)
})
...
test('Code and value as string', () => {
  expect(
    messageHandler({
      'type': 'message',
      'origin': document.location.origin,
      'data': { 'command': 'ipfs', 'code': `aa100`, 'value': 'a1' }
    })
  ).toStrictEqual(false)
})
