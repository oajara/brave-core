...
export function useTransactionFeesParser(
   ...
   * no error.
   */
  const checkForMissingGasLimitError = React.useCallback(
  ...
  return React.useCallback(
    (transactionInfo: BraveWallet.TransactionInfo): ParsedTransactionFees => {
      const {
        txDataUnion: { ethTxData1559: txData, filTxData }
      } = transactionInfo
      ...
      const gasFee = isSolanaTxn
        ...
        ? new Amount(maxFeePerGas).times(gasLimit).format()
        : new Amount(gasPrice).times(gasLimit).format()
      ...
      return {
        ...
        gasPremium: isFilTransaction
          ? new Amount(filTxData.gasPremium).format()
          : '',
        gasFeeCap: isFilTransaction
          ? new Amount(filTxData.gasFeeCap).format()
          : ''
      }
    },
    [selectedNetwork, networkSpotPrice]
...
export function useTransactionParser(
  ...
  const networkSpotPrice = React.useMemo(
    () => (selectedNetwork ? findAssetPrice(selectedNetwork.symbol) : ''),
  ...
  const findToken = React.useCallback(
    (contractAddress: string) => {
      const checkVisibleList = visibleTokens.find(
        (token) =>
  ...
  const checkForContractAddressError = (to: string): string | undefined => {
    return fullTokenList?.some(
      (token) => token.contractAddress.toLowerCase() === to.toLowerCase()
    )
   ...
   */
  const checkForSameAddressError = (
    to: string,
    from: string
  ): string | undefined => {
  ...
  return React.useCallback(
    (transactionInfo: BraveWallet.TransactionInfo): ParsedTransaction => {
      const {
        txArgs,
        txDataUnion: {
      ...
      switch (true) {
        ...
        case txType === BraveWallet.TransactionType.Other &&
          solTxData !== undefined: {
          const instructions = solTxData
            ? getTypedSolanaTxInstructions(solTxData)
            : []

          const lamportsMovedFromInstructions =
            instructions.reduce((acc, { type, params }) => {
              const lamportsAmount =
                (params as SolanaParamsWithLamports)?.lamports?.toString() ??
                '0'

              switch (type) {
                case 'Transfer':
                case 'TransferWithSeed': {
                  const { fromPubkey, toPubkey } = params as
                    | Solana.TransferParams
                    | Solana.TransferWithSeedParams
                ...
                case 'WithdrawNonceAccount': {
                  const { noncePubkey, toPubkey } =
                    params as Solana.WithdrawNonceParams
                ...
                case 'Create':
                case 'CreateWithSeed': {
                  const { fromPubkey, newAccountPubkey } = params as
                    | Solana.CreateAccountParams
                    | Solana.CreateAccountWithSeedParams
                  if (!to) {
                ...
                case 'Unknown': {
                  if (!to) {
                    to =
                      solTxData?.instructions[0]?.accountMetas[0]?.pubkey.toString() ??
                      ''
                  }
                }
                default:
          ...
          const totalAmountFiat = new Amount(gasFeeFiat).plus(
            transferedAmountFiat
          )
        ...
        case txType === BraveWallet.TransactionType.ERC20Transfer: {
          ...
          const totalAmountFiat = new Amount(gasFeeFiat).plus(sendAmountFiat)

          const insufficientNativeFunds =
            accountNativeBalance !== ''
              ? new Amount(gasFee).gt(accountNativeBalance)
              : undefined
          ...
          const valueWrapped = new Amount(amount).divideByDecimals(
            token?.decimals ?? 18
          )
          ...
          return {
            ...
            intent: getLocale('braveWalletTransactionIntentSend').replace(
              '$1',
        ...
        case txType ===
          ...
          const valueWrapped = new Amount(value).divideByDecimals(
            token?.decimals ?? 9
          )
          ...
          return {
            ...
            insufficientFundsError: insufficientTokenFunds,
            insufficientFundsForGasError: insufficientNativeFunds,
            contractAddressError: checkForContractAddressError(
              solTxData?.toWalletAddress ?? ''
            ),
            sameAddressError: checkForSameAddressError(
              solTxData?.toWalletAddress ?? '',
              fromAddress
            ),
            intent: getLocale('braveWalletTransactionIntentSend').replace(
        ...
        case txType === BraveWallet.TransactionType.ETHSwap: {
          ...
          const sellToken =
            fillTokens.length === 1 ? nativeAsset : fillTokens[0]
          ...
          const buyAmount = new Amount(minBuyAmountArg).divideByDecimals(
            buyToken.decimals
          )
          ...
          const sellAmountBN = sellToken
            ? sellAmountWeiBN.divideByDecimals(sellToken.decimals)
            : Amount.empty()
        ...
        default: {
          ...
          return {
            ...
            insufficientFundsError:
              accountNativeBalance !== ''
                ? new Amount(value).plus(gasFee).gt(accountNativeBalance)
                : undefined,
            insufficientFundsForGasError:
              accountNativeBalance !== ''
                ? new Amount(gasFee).gt(accountNativeBalance)
            ...
            intent: getLocale('braveWalletTransactionIntentSend').replace(
              '$1',
              valueWrapped.formatAsAsset(6, selectedNetwork?.symbol)
            ),
            ...feeDetails
          } as ParsedTransaction
        }
