...
function makeSendOptions(
  maxRetries?: number,
  preflightCommitment?: string,
  skipPreflight?: boolean
): BraveWallet.SolanaSendTransactionOptions {
  return {
    maxRetries:
    ...
    preflightCommitment,
    skipPreflight:
      skipPreflight !== undefined ? { skipPreflight: skipPreflight } : undefined
  }
}

export default function useSwap({
  fromAsset: fromAssetProp,
  toAsset: toAssetProp
}: Args = {}) {
  // redux
  const {
  ...
  const [filteredAssetList, setFilteredAssetList] = React.useState<
  ...
  const [orderExpiration, setOrderExpiration] =
    React.useState<ExpirationPresetObjectType>(ExpirationPresetOptions[0])
  ...
  const [slippageTolerance, setSlippageTolerance] =
    React.useState<SlippagePresetObjectType>(
  ...
  // evm swaps (0x) state
  const [allowance, setAllowance] = React.useState<string | undefined>(
    undefined
  )
  const [swapError, setSwapError] = React.useState<
    SwapErrorResponse | undefined
  >(undefined)
  const [swapQuote, setSwapQuote] = React.useState<
    BraveWallet.SwapResponse | undefined
  >(undefined)

  // solana swaps (jupiter) state
  const [jupiterQuote, setJupiterQuote] = React.useState<
    BraveWallet.JupiterQuote | undefined
  >(undefined)
  const [jupiterError, setJupiterError] = React.useState<
    JupiterErrorResponse | undefined
  >(undefined)
  ...
  const swapAssetOptions: BraveWallet.BlockchainToken[] = React.useMemo(() => {
    ...
    return [
      ...
      ...userVisibleTokensInfo.filter(
        (asset) =>
          !['USDC', nativeAsset.symbol.toUpperCase()].includes(
        ...
        .filter(
          (asset) =>
            !userVisibleTokensInfo.some(
              (token) =>
                token.symbol.toUpperCase() === asset.symbol.toUpperCase()
            )
        )
    ].filter(
      (asset) =>
        asset.chainId === selectedNetwork.chainId && !asset.isErc721 // NFT swaps not supported
    )
  ...
  const swapValidationError: SwapValidationErrorType | undefined =
    React.useMemo(() => {
      ...
      if (
        fromAsset.chainId === selectedNetwork.chainId &&
        fromAsset.coin === selectedNetwork.coin &&
          ...
          case SWAP_VALIDATION_ERROR_CODE:
            if (
              validationErrors?.find(
                (err) => err.reason === 'INSUFFICIENT_ASSET_LIQUIDITY'
              )
            ) {
  ...
  const isSwapButtonDisabled = React.useMemo(() => {
    return (
      ...
      (swapValidationError &&
        swapProvider === SwapProvider.ZeroEx &&
  ...
  const setToAssetAndMakeVisible = React.useCallback(
    (token?: BraveWallet.BlockchainToken) => {
      setToAsset(token)
  ...
  const fetch0xSwapQuote = React.useCallback(
    async (payload: SwapParamsPayloadType) => {
      if (!isMounted) {
        return
      }
      ...
      const swapParams: BraveWallet.SwapParams = {
        ...
        sellToken:
          fromAsset.contractAddress || NATIVE_ASSET_CONTRACT_ADDRESS_0X,
        slippagePercentage: slippageTolerance.slippage / 100,
        gasPrice: ''
      } as BraveWallet.SwapParams

      const quote = await (full
        ? swapService.getTransactionPayload(swapParams)
        : swapService.getPriceQuote(swapParams))
      ...
      if (quote.success && quote.response) {
        ...
        if (full) {
          ...
          if (
            isEIP1559 &&
            gasEstimates &&
            gasEstimates.fastMaxPriorityFeePerGas ===
              gasEstimates.avgMaxPriorityFeePerGas
          ) {
            // Bump fast priority fee and max fee by 1 GWei if same as average fees.
            const maxPriorityFeePerGasBN = new BigNumber(
              gasEstimates.fastMaxPriorityFeePerGas
            ).plus(10 ** 9)
            const maxFeePerGasBN = new BigNumber(
              gasEstimates.fastMaxFeePerGas
            ).plus(10 ** 9)
  ...
  const fetchJupiterSwapQuote = React.useCallback(
    async (payload: SwapParamsPayloadType) => {
      if (!isMounted) {
        return
      }
      ...
      if (!full) {
        ...
        if (quote.success && quote.response) {
          ...
          try {
            ...
            console.error(
              `[swap] error querying Jupiter API: ${quote.errorResponse}`
            )
          }
        }
      } else {
        ...
        if (swapTransactions.success && swapTransactions.response) {
          ...
          await Promise.all(
            serializedTransactions.map(async (each, idx) => {
          ...
          try {
            const err = JSON.parse(
              swapTransactions.errorResponse
            ...
            console.error(
              `[swap] error querying Jupiter swap API: ${swapTransactions.errorResponse}`
            )
          }
        }
      }
  ...
  const onSwapParamsChange = React.useCallback(
    async ({ full = false, overrides, state }: OnSwapParamChangeArgs) => {
      /**
      ...
      if (overrides.toOrFrom === 'from') {
        ...
        fromAmountWeiWrapped = new Amount(
          overrides.amount ?? state.fromAmount
      ...
      if (overrides.toOrFrom === 'to') {
        if (
          hasDecimalsOverflow(overrides.amount ?? state.toAmount, toAssetNext)
        ) {
          return
        }

        if (overrides.toAsset === undefined) {
          toAmountWeiWrapped = new Amount(
      ...
      if (
        fromAmountWeiWrapped?.isUndefined() ||
        fromAmountWeiWrapped?.isZero()
      ) {
        setFromAmount('')
        return
      }
  ...
  const flipSwapAssets = React.useCallback(() => {
    ...
    toAmount,
    onSwapParamsChange,
    setToAssetAndMakeVisible
  ])

  const onSetFromAmount = React.useCallback(
    async (value: string) => {
      setFromAmount(value)
  ...
  const onSetToAmount = React.useCallback(
    async (value: string) => {
      setToAmount(value)
      await onSwapParamsChangeDebounced({
  ...
  const onCustomSlippageToleranceChange = React.useCallback(
    (value: string) => {
  ...
  const onSelectSlippageTolerance = React.useCallback(
  ...
  const onSubmitSwap = React.useCallback(() => {
    ...
    if (swapProvider === SwapProvider.ZeroEx) {
      ...
      if (
        ...
        // amount.
        const allowanceHex = new Amount(MAX_UINT256).toHex()
  ...
  const onSelectTransactAsset = React.useCallback(
    (asset: BraveWallet.BlockchainToken, toOrFrom: ToOrFromType) => {
      if (toOrFrom === 'from') {
        setFromAsset(asset)
      } else {
  ...
  const onSwapInputChange = React.useCallback(
    (value: string, name: 'to' | 'from' | 'rate') => {
      if (name === 'to') {
        onSetToAmount(value)
      }
      if (name === 'from') {
  ...
  React.useEffect(() => {
    ...
    getIsSwapSupported(selectedNetwork)
      .then((supported) => {
        if (isSubscribed) {
          try {
  ...
  React.useEffect(() => {
    ...
    const approxPriceWrapped = new Amount(buyTokenToEthRate).div(
      sellTokenToEthRate
    )
  ...
  React.useEffect(() => {
    ...
    const newFromAmountWrapped = new Amount(
      inAmount.toString()
    ).divideByDecimals(fromAsset.decimals)
    setFromAmount(newFromAmountWrapped.format())

    const newToAmountWrapped = new Amount(
  ...
  React.useEffect(
    () => setIsLoading(false),
    [swapQuote, jupiterQuote, swapError, jupiterError]
  )
