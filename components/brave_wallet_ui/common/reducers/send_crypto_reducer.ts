...
export const SendCryptoActions = {
  ...
  setShowEnsOffchainLookupOptions: createAction<boolean>(
    'setShowEnsOffchainLookupOptions'
  ),
  setEnsOffchainLookupOptions: createAction<
    BraveWallet.EnsOffchainLookupOptions | undefined
  >('setEnsOffchainLookupOptions'),
  setAddressWarning: createAction<string | undefined>('setAddressWarning'),
  selectSendAsset: createAction<BraveWallet.BlockchainToken | undefined>(
    'selectSendAsset'
  )
}

export const createSendCryptoReducer = (
  initialState: PendingCryptoSendState
) => {
  const reducer = createReducer<PendingCryptoSendState>({}, initialState)

  reducer.on(
    SendCryptoActions.setSendAmount,
    (
  ...
  reducer.on(
    SendCryptoActions.setToAddressOrUrl,
    (
      state: PendingCryptoSendState,
      payload: string
    ): PendingCryptoSendState => {
  ...
  reducer.on(
    SendCryptoActions.setAddressError,
    (
      state: PendingCryptoSendState,
      payload: string
    ): PendingCryptoSendState => {
  ...
  reducer.on(
    SendCryptoActions.setToAddress,
    (
      state: PendingCryptoSendState,
      payload: string
    ): PendingCryptoSendState => {
  ...
  reducer.on(
    SendCryptoActions.setShowEnsOffchainLookupOptions,
    (
      state: PendingCryptoSendState,
      payload: boolean
    ): PendingCryptoSendState => {
  ...
  reducer.on(
    SendCryptoActions.setEnsOffchainLookupOptions,
    (
      state: PendingCryptoSendState,
  ...
  reducer.on(
    SendCryptoActions.setAddressWarning,
    (
      state: PendingCryptoSendState,
      payload: string
    ): PendingCryptoSendState => {
  ...
  reducer.on(
    SendCryptoActions.selectSendAsset,
    (
      state: PendingCryptoSendState,
