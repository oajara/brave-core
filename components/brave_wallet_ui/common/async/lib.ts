...
export const getBalance = (
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (coin === BraveWallet.CoinType.SOL) {
      const result = await jsonRpcService.getSolanaBalance(
        address,
        chainId.chainId
      )
    ...
    const result = await jsonRpcService.getBalance(
      address,
      coin,
      chainId.chainId
    )
...
export async function findENSAddress(
  address: string,
  ensOffchainLookupOptions?: BraveWallet.EnsOffchainLookupOptions | undefined
) {
  const apiProxy = getAPIProxy()
  return apiProxy.jsonRpcService.ensGetEthAddr(
    address,
    ensOffchainLookupOptions || null
  )
}
...
export async function findUnstoppableDomainAddress(
  address: string,
...
export async function getBuyAssets(
  ...
  const { blockchainRegistry } = getAPIProxy()
  return (await blockchainRegistry.getBuyTokens(onRampProvider, chainId)).tokens
}
...
export const getAllBuyAssets = async (): Promise<{
  ...
  const rampAssetsPromises = await Promise.all(
    SupportedOnRampNetworks.map((chainId) =>
      blockchainRegistry.getBuyTokens(kRamp, chainId)
    )
  )
  const wyreAssetsPromises = await Promise.all(
    SupportedOnRampNetworks.map((chainId) =>
      blockchainRegistry.getBuyTokens(kWyre, chainId)
    )
  )
  const sardineAssetsPromises = await Promise.all(
    SupportedOnRampNetworks.map((chainId) =>
      blockchainRegistry.getBuyTokens(kSardine, chainId)
    )
  )
  ...
  const sardineAssetOptions: BraveWallet.BlockchainToken[] =
    sardineAssetsPromises.flatMap((p) => p.tokens).map(addLogoToToken)

  // seperate native assets from tokens
  const { tokens: rampTokenOptions, nativeAssets: rampNativeAssetOptions } =
    getNativeTokensFromList(rampAssetOptions)

  const { tokens: wyreTokenOptions, nativeAssets: wyreNativeAssetOptions } =
  ...
  const { bat: wyreBatTokens, nonBat: wyreNonBatTokens } =
  ...
  const sortedWyreOptions = [
    ...wyreNativeAssetOptions,
    ...wyreBatTokens,
    ...wyreNonBatTokens
  ]
...
export function getKeyringIdFromCoin(
  coin: BraveWallet.CoinType
): BraveKeyrings {
...
export async function getKeyringIdFromAddress(
  address: string
): Promise<string> {
  const apiProxy = getAPIProxy()
  const result = await apiProxy.walletHandler.getWalletInfo()
...
export async function getIsSwapSupported(
...
function reportActiveWalletsToP3A(
  ...
  for (const [coin, coinActiveAddresses] of Object.entries(
    coinsActiveAddresses
  )) {
...
export function refreshBalances() {
  return async (dispatch: Dispatch, getState: () => State) => {
    const { jsonRpcService } = getAPIProxy()
    const {
      wallet: { accounts, userVisibleTokensInfo, networkList }
    } = getState()
    ...
    const getNativeAssetsBalanceReturnInfos = await Promise.all(
            ...
            if (account.coin === BraveWallet.CoinType.FIL) {
              if (networkList.some((n) => n.chainId === network.chainId)) {
                ...
                if (
                  ...
                  account.keyringId === getFilecoinKeyringIdFromNetwork(network)
                ) {
    ...
    await dispatch(
      WalletActions.nativeAssetBalancesUpdated({
        balances: getNativeAssetsBalanceReturnInfos
      })
    )

    const visibleTokens = userVisibleTokensInfo.filter(
      (asset) => asset.contractAddress !== ''
    )

    const getBlockchainTokensBalanceReturnInfos = await Promise.all(
      accounts.map(async (account) => {
        const networks = getNetworksByCoinType(networkList, account.coin)
    ...
    await dispatch(
      WalletActions.tokenBalancesUpdated({
...
export function refreshPrices() {
  return async (dispatch: Dispatch, getState: () => State) => {
    ...
    const mainnetList = networkList.filter(
      (network) => !SupportedTestNetworks.includes(network.chainId)
    )
    const getNativeAssetPrices = await Promise.all(
      mainnetList.map(async (network) => {
        const getNativeAssetPrice = await assetRatioService.getPrice(
          [network.symbol.toLowerCase()],
          [defaultFiatCurrency],
          selectedPortfolioTimeline
        )
        const nativeAssetPrice = getNativeAssetPrice.success
          ? getNativeAssetPrice.values.find(
              (i) => i.toAsset === defaultFiatCurrency
            )?.price ?? ''
          : ''
        return {
    ...
    const blockChainTokenInfo = userVisibleTokensInfo.filter(
      (token) => token.contractAddress !== ''
    )
    const getTokenPrices = await Promise.all(
      getFlattenedAccountBalances(accounts, blockChainTokenInfo).map(
        async (token) => {
          const emptyPrice = {
          ...
          const price =
            token.balance > 0 && !token.token.isErc721
    ...
    await dispatch(
      WalletActions.pricesUpdated({
        success: true,
        values: [...getNativeAssetPrices, ...getTokenPrices]
      })
    )
  }
}

export function refreshTokenPriceHistory(
      ...
      return filteredTokenInfo
        ...
        .map((token) => {
          ...
          return {
            token,
            balance: balance || '0',
            history: priceHistory.find(
              (t) => contractAddress === t.contractAddress
            )?.history ?? { success: true, values: [] }
          }
        })
    })
    dispatch(
      WalletActions.portfolioPriceHistoryUpdated(priceHistoryWithBalances)
    )
  }
}
...
export function refreshTransactionHistory(address?: string) {
  return async (dispatch: Dispatch, getState: () => State) => {
    ...
    const freshTransactions: AccountTransactions =
      await accountsToUpdate.reduce(
        async (acc, account) =>
          acc.then(async (obj) => {
            ...
            obj[account.address] = sortTransactionByDate(
              transactionInfos,
              'descending'
            )
            return obj
    ...
    dispatch(
      WalletActions.setAccountTransactions({
        ...transactions,
        ...freshTransactions
      })
    )
...
export function refreshFullNetworkList() {
  return async (dispatch: Dispatch, getState: () => State) => {
    ...
    const {
      wallet: { isFilecoinEnabled, isSolanaEnabled }
    } = getState()

    // Get isTestNetworkEnabled
    const isTestNetworksEnabled =
      await braveWalletService.getShowWalletTestNetworks()

    // Get All Networks
    const getFullNetworkList = await Promise.all(
    ...
    let networkList = isTestNetworksEnabled.isEnabled
      ? flattenedNetworkList
      : flattenedNetworkList.filter(
          (network) => !SupportedTestNetworks.includes(network.chainId)
        )

    const defaultEthChainId = (
      await jsonRpcService.getChainId(BraveWallet.CoinType.ETH)
    ).chainId
    const hiddenEthNetworkList = (
...
export function refreshNetworkInfo() {
  return async (dispatch: Dispatch, getState: () => State) => {
    ...
    const {
      wallet: { selectedCoin, networkList }
    } = getState()

    // Get default network for each coinType
    const defaultNetworks = await Promise.all(
      SupportedCoinTypes.map(async (coin: BraveWallet.CoinType) => {
  ...
  return async (dispatch: Dispatch, getState: () => State) => {
    ...
    const defaultAccounts = await Promise.all(
      SupportedCoinTypes.map(async (coin: BraveWallet.CoinType) => {
        ...
        return (
          walletInfo.accountInfos.find(
            (account) =>
              account.address.toLowerCase() ===
              defaultAccountAddress?.toLowerCase()
          ) ?? ({} as BraveWallet.AccountInfo)
        )
      })
    )
    const filteredDefaultAccounts = defaultAccounts.filter(
    ...
    if (!selectedAddress) {
      await keyringService.setSelectedAccount(
        fallbackAccount.address,
        fallbackAccount.coin
      )
...
export function refreshSitePermissions() {
  return async (dispatch: Dispatch, getState: () => State) => {
    ...
    const {
      wallet: { accounts, activeOrigin }
    } = getState()

    // Get a list of accounts with permissions of the active origin
    const getAllPermissions = await Promise.all(
      accounts.map(async (account) => {
    ...
    const accountsWithPermission: WalletAccountType[] =
      getAllPermissions.filter(
        (account): account is WalletAccountType => account !== undefined
      )
    dispatch(
 ...
 */
export function hasEIP1559Support(
