...
async function refreshBalancesPricesAndHistory(store: Store) {
  ...
  await store.dispatch(
    refreshTokenPriceHistory(state.selectedPortfolioTimeline)
  )
}
...
async function updateCoinAccountNetworkInfo(
  ...
  const defaultAccount =
    accounts.find(
      (account) => account.address === selectedAccountAddress.address
    ) || accounts[0]
...
handler.on(
  WalletActions.refreshBalancesAndPrices.type,
  async (store: Store) => {
    await store.dispatch(refreshVisibleTokenInfo())
    await store.dispatch(refreshBalances())
    await store.dispatch(refreshPrices())
...
handler.on(
  WalletActions.selectNetwork.type,
  async (store: Store, payload: BraveWallet.NetworkInfo) => {
...
handler.on(
  WalletActions.chainChangedEvent.type,
  async (store: Store, payload: ChainChangedEventPayloadType) => {
...
handler.on(
  ...
  async (store: Store, payload: WalletAccountType) => {
...
handler.on(
  WalletActions.selectedAccountChanged.type,
  async (store, payload: SelectedAccountChangedPayloadType) => {
    await updateCoinAccountNetworkInfo(store, payload.coin)
  }
)
...
handler.on(
  WalletActions.initialized.type,
...
handler.on(WalletActions.getAllTokensList.type, async (store) => {
  ...
  const getAllTokensList = await Promise.all(
...
handler.on(
  WalletActions.sendTransaction.type,
  async (
    store: Store,
    payload:
      | SendEthTransactionParams
...
handler.on(
  WalletActions.sendSPLTransfer.type,
  async (store: Store, payload: SPLTransferFromParams) => {
    ...
    if (!value.txData) {
      console.log(
        'Failed making SPL transfer data, to: ',
        payload.to,
        ', value: ',
        payload.value
      )
...
handler.on(
  WalletActions.sendERC20Transfer.type,
  async (store: Store, payload: ER20TransferParams) => {
    const apiProxy = getAPIProxy()
    const { data, success } =
      await apiProxy.ethTxManagerProxy.makeERC20TransferData(
        payload.to,
        payload.value
      )
    if (!success) {
...
handler.on(
  WalletActions.sendERC721TransferFrom.type,
  async (store: Store, payload: ERC721TransferFromParams) => {
    const apiProxy = getAPIProxy()
    const { data, success } =
      await apiProxy.ethTxManagerProxy.makeERC721TransferFromData(
        payload.from,
        payload.to,
        payload.tokenId,
        payload.contractAddress
    ...
    if (!success) {
      console.log(
        'Failed making ERC721 transferFrom data, from: ',
        payload.from,
        ', to: ',
        payload.to,
        ', tokenId: ',
...
handler.on(
  WalletActions.approveERC20Allowance.type,
  async (store: Store, payload: ApproveERC20Params) => {
    const apiProxy = getAPIProxy()
    const { data, success } =
      await apiProxy.ethTxManagerProxy.makeERC20ApproveData(
...
handler.on(
  WalletActions.approveTransaction.type,
  async (store: Store, txInfo: BraveWallet.TransactionInfo) => {
    ...
    const error =
      result.errorUnion.providerError ?? result.errorUnion.solanaProviderError
    if (error !== BraveWallet.ProviderError.kSuccess) {
      await store.dispatch(
        WalletActions.setTransactionProviderError({
          transaction: txInfo,
          providerError: {
            code: error,
...
handler.on(
  WalletActions.rejectTransaction.type,
  async (store: Store, txInfo: BraveWallet.TransactionInfo) => {
    const apiProxy = getAPIProxy()
    const coin = getCoinFromTxDataUnion(txInfo.txDataUnion)
...
handler.on(
  WalletActions.refreshGasEstimates.type,
  async (store: Store, txInfo: BraveWallet.TransactionInfo) => {
...
handler.on(
  WalletActions.updateUnapprovedTransactionGasFields.type,
  async (store: Store, payload: UpdateUnapprovedTransactionGasFieldsType) => {
    const apiProxy = getAPIProxy()
    ...
    if (isEIP1559) {
      const result =
        await apiProxy.ethTxManagerProxy.setGasFeeAndLimitForUnapprovedTransaction(
          payload.txMetaId,
    ...
    if (!isEIP1559 && payload.gasPrice) {
      const result =
        await apiProxy.ethTxManagerProxy.setGasPriceAndLimitForUnapprovedTransaction(
          payload.txMetaId,
          payload.gasPrice,
          payload.gasLimit
        )
...
handler.on(
  WalletActions.updateUnapprovedTransactionSpendAllowance.type,
  async (
    store: Store,
    payload: UpdateUnapprovedTransactionSpendAllowanceType
  ) => {
    const apiProxy = getAPIProxy()
...
handler.on(
  WalletActions.updateUnapprovedTransactionNonce.type,
  async (store: Store, payload: UpdateUnapprovedTransactionNonceType) => {
    const { ethTxManagerProxy } = getAPIProxy()

    const result = await ethTxManagerProxy.setNonceForUnapprovedTransaction(
      payload.txMetaId,
      payload.nonce
    )
    if (!result.success) {
...
handler.on(
  ...
  async (store: Store, payload: RemoveSitePermissionPayloadType) => {
    const braveWalletService = getAPIProxy().braveWalletService
    await braveWalletService.resetPermission(
      payload.coin,
      payload.origin,
      payload.account
    )
...
handler.on(
  WalletActions.addSitePermission.type,
  async (store: Store, payload: AddSitePermissionPayloadType) => {
    const braveWalletService = getAPIProxy().braveWalletService
    await braveWalletService.addPermission(
      payload.coin,
      payload.origin,
      payload.account
    )
...
handler.on(
  WalletActions.transactionStatusChanged.type,
  async (store: Store, payload: TransactionStatusChanged) => {
...
handler.on(
  ...
  async (store: Store, payload: BraveWallet.TransactionInfo) => {
...
handler.on(
  WalletActions.setSelectedNetworkFilter.type,
  async (store: Store, payload: BraveWallet.NetworkInfo) => {
    const state = getWalletState(store)
...
handler.on(
  WalletActions.setSelectedAccountFilterItem.type,
  async (store: Store, payload: BraveWallet.NetworkInfo) => {
    const state = getWalletState(store)
    const { selectedPortfolioTimeline } = state
    await store.dispatch(refreshTokenPriceHistory(selectedPortfolioTimeline))
...
handler.on(
  WalletActions.addAccount.type,
  async (_store: Store, payload: AddAccountPayloadType) => {
    const { keyringService } = getAPIProxy()
    const result = await keyringService.addAccount(
      payload.accountName,
      payload.coin
    )
...
handler.on(
  WalletActions.addFilecoinAccount.type,
  async (_store: Store, payload: AddFilecoinAccountPayloadType) => {
    const { keyringService } = getAPIProxy()
    const result = await keyringService.addFilecoinAccount(
      payload.accountName,
      payload.network
    )
    return result.success
