...
export const Row = styled.div<Partial<StyleProps>>`
  ...
  align-items: center;
  justify-content: ${(p) =>
    p.componentType === 'exchange' ? 'flex-start' : 'space-between'};
`
...
export const SlippageInput = styled.input<Partial<StyleProps>>`
  ...
  border: 1px solid
    ${(p) =>
      p.isSelected ? p.theme.color.interactive05 : p.theme.color.interactive08};
  color: ${(p) =>
    p.isSelected ? p.theme.palette.white : p.theme.color.text03};
...
export const WarningText = styled.span<Partial<StyleProps>>`
  ...
  font-size: 12px;
  color: ${(p) =>
    p.isWarning ? p.theme.color.interactive03 : p.theme.color.errorText};
