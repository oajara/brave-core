...
export const TabButton = styled(WalletButton)<Partial<StyleProps>>`
  ...
  border: ${(p) =>
    p.isSelected
      ? `2px solid ${p.theme.color.divider01}`
      : `2px solid ${p.theme.color.background01}`};
  border-bottom-width: 0px;
  z-index: ${(p) => (p.isSelected ? '2' : '0')};
  position: relative;
...
export const RightDivider = styled.div<Partial<StyleProps>>`
  ...
  width: 1px;
  background-color: ${(p) =>
    p.selectedTab === 'buy' && p.tabID === 'send'
...
export const LeftDivider = styled.div<Partial<StyleProps>>`
  ...
  width: 1px;
  background-color: ${(p) =>
    p.selectedTab === 'buy' && p.tabID === 'swap'
      ? p.theme.color.divider01
