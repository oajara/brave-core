...
export const ImportAccountModal = () => {
  ...
  // redux
  const isSolanaEnabled = useSelector(
    ({ wallet }: { wallet: WalletState }) => wallet.isSolanaEnabled
  )
  const isFilecoinEnabled = useSelector(
    ({ wallet }: { wallet: WalletState }) => wallet.isFilecoinEnabled
  )
  ...
  const selectedAccountType: CreateAccountOptionsType | undefined =
    React.useMemo(() => {
      ...
      return CreateAccountOptions(isFilecoinEnabled, isSolanaEnabled).find(
        (option) => {
  ...
  const hasImportError = useSelector(
    ({ page }: { page: PageState }) => page.importAccountError
  )
  ...
  const importAccount = React.useCallback(
    (accountName: string, privateKey: string, coin: BraveWallet.CoinType) => {
      dispatch(
        WalletPageActions.importAccount({ accountName, privateKey, coin })
      )
    },
    []
  )
  ...
  const importFilecoinAccount = React.useCallback(
    (accountName: string, privateKey: string, network: string) => {
      dispatch(
        WalletPageActions.importFilecoinAccount({
          accountName,
          privateKey,
          network
        })
  ...
  const importAccountFromJson = React.useCallback(
    (accountName: string, password: string, json: string) => {
      dispatch(
        WalletPageActions.importAccountFromJson({ accountName, password, json })
      )
    },
    []
  )
    ...
    <PopupModal title={modalTitle} onClose={onClickClose}>
      ...
      {selectedAccountType && (
        <StyledWrapper>
          ...
          {selectedAccountType?.coin === BraveWallet.CoinType.ETH && (
            <SelectWrapper>
              <Select value={importOption} onChange={setImportOption}>
                <div data-value="key">
