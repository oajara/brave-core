...
export const PortfolioTransactionItem = React.forwardRef<HTMLDivElement, Props>(
  (
    { transaction, account, displayAccountName, accounts, isFocused }: Props,
    forwardedRef
  ) => {
    // routing
    ...
    const { defaultCurrencies, userVisibleTokensInfo } = useSelector(
    ...
    const findWalletAccount = React.useCallback(
      (address: string) => {
    ...
    const findToken = React.useCallback(
      (symbol: string) => {
        return userVisibleTokensInfo.find(
          ...
          return (
            <DetailRow>
              <DetailTextDark>
                ...
                -{' '}
                <AddressOrAsset
                  onClick={onAddressClick(transactionDetails.approvalTarget)}
                >
          ...
          return (
            <DetailRow>
              ...
              <DetailTextDark>
                ...
                <AddressOrAsset
                  onClick={onAddressClick(transactionDetails.buyToken?.symbol)}
                >
        ...
        case transaction.txType !== BraveWallet.TransactionType.ETHSwap &&
          ...
          return (
            <DetailRow>
              ...
              <DetailTextDark>
                <AddressOrAsset
                  onClick={onAddressClick(transactionDetails.recipient)}
                >
        ...
        default: {
          return (
            <DetailRow>
              <DetailTextDark>
                <AddressOrAsset
                  onClick={onAddressClick(transactionDetails.sender)}
                >
    ...
    const transactionActionLocale = React.useMemo(() => {
      switch (true) {
        ...
        case transaction.txDataUnion.ethTxData1559?.baseData.to.toLowerCase() ===
          SwapExchangeProxy: {
          ...
          <DetailRow>
            ...
            {showTransactionPopup && (
              <TransactionPopup>
                ...
                ].includes(transactionDetails.status) && (
                  <>
                ...
                {[BraveWallet.TransactionStatus.Error].includes(
