...
const TransactionDetailPanel = (props: Props) => {
  ...
  // redux
  const { transactions, transactionProviderErrorRegistry, defaultNetworks } =
  ...
  const liveTransaction = React.useMemo(() => {
    return (
      transactionsList.find((tx) => tx.id === transaction.id) ?? transaction
    )
  ...
  const transactionValue = React.useMemo((): string => {
    ...
    return new Amount(transactionDetails.value).formatAsAsset(
      undefined,
  ...
  const transactionFiatValue = React.useMemo((): string => {
    if (
      ...
      return transactionDetails.fiatValue.formatAsFiat(defaultCurrencies.fiat)
    }
    return ''
  ...
  const isFilecoinTransaction =
    getCoinFromTxDataUnion(liveTransaction.txDataUnion) ===
    ...
    <StyledWrapper>
          ...
          {transactionDetails.status === BraveWallet.TransactionStatus.Error &&
            transactionProviderErrorRegistry[liveTransaction.id] && (
              <TransactionStatusTooltip
                text={`${
                  transactionProviderErrorRegistry[liveTransaction.id].code
                }: ${
                  transactionProviderErrorRegistry[liveTransaction.id].message
                }`}
              >
                <AlertIcon />
              </TransactionStatusTooltip>
      ...
      {![
        ...
        <DetailRow>
          ...
          <DetailButton
            onClick={onClickViewOnBlockExplorer('tx', liveTransaction?.txHash)}
          >
