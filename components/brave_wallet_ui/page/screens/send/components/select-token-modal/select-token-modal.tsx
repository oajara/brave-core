  ...
  (props: Props, forwardedRef) => {
    ...
    const getTokenListByAccount = React.useCallback(
      (account: WalletAccountType) => {
        ...
        if (
          accountsLocalHost &&
          account.keyringId !== BraveWallet.FILECOIN_KEYRING_ID
        ) {
    ...
    const getTokenListWithBalances = React.useCallback(
      (account: WalletAccountType) => {
        return getTokenListByAccount(account).filter(
          (token) => getBalance(account, token) !== '0'
        )
      },
    ...
    const tokensByAccount = React.useMemo(() => {
      if (emptyTokensList) {
        return (
          <Text textSize="14px" isBold={false} textColor="text03">
