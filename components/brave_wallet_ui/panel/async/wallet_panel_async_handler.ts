...
handler.on(
  PanelActions.cancelConnectToSite.type,
  async (store: Store, payload: AccountPayloadType) => {
    const apiProxy = getWalletPanelApiProxy()
...
handler.on(
  ...
  async (store: Store, payload: CancelConnectHardwareWalletPayload) => {
...
handler.on(
  PanelActions.approveHardwareTransaction.type,
    ...
    if (hardwareAccount.vendor === BraveWallet.LEDGER_HARDWARE_VENDOR) {
      ...
      switch (found.coin) {
        case BraveWallet.CoinType.ETH:
          ;({ success, error, code } = await signLedgerEthereumTransaction(
            apiProxy,
        ...
        case BraveWallet.CoinType.FIL:
          ;({ success, error, code } = await signLedgerFilecoinTransaction(
            apiProxy,
            txInfo,
            found.coin
          ))
...
handler.on(
  PanelActions.showApproveTransaction.type,
  async (store: Store, payload: ShowConnectToSitePayload) => {
...
handler.on(
  ...
  async (store, messageData: BraveWallet.SignMessageRequest) => {
    const apiProxy = getWalletPanelApiProxy()
    ...
    if (signed.success) {
      ...
      signature =
        hardwareAccount.coin === BraveWallet.CoinType.SOL
...
handler.on(
  ...
  async (store, messageData: BraveWallet.SignTransactionRequest) => {
    const apiProxy = getWalletPanelApiProxy()
    const hardwareAccount = await findHardwareAccountInfo(
...
handler.on(
  ...
  async (store, messageData: BraveWallet.SignAllTransactionsRequest) => {
    const apiProxy = getWalletPanelApiProxy()
    const hardwareAccount = await findHardwareAccountInfo(
      messageData.fromAddress
    )
    ...
    let payload: SignAllTransactionsProcessedPayload = {
      approved: true,
      id: messageData.id,
      signatures: []
    }
...
handler.on(
  ...
  async (store: Store, payload: BraveWallet.AddSuggestTokenRequest[]) => {
...
handler.on(
  PanelActions.addSuggestTokenProcessed.type,
  async (store: Store, payload: AddSuggestTokenProcessedPayload) => {
    const apiProxy = getWalletPanelApiProxy()
    const braveWalletService = apiProxy.braveWalletService
    braveWalletService.notifyAddSuggestTokenRequestsProcessed(
      payload.approved,
