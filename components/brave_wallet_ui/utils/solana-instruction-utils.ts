...
export const getSolanaTransactionInstructionParamsAndType = ({
  ...
  const instruction: Solana.TransactionInstruction =
  ...
  if (instruction.programId.equals(Solana.SystemProgram.programId)) {
    instructionType =
      Solana.SystemInstruction.decodeInstructionType(instruction)
    params =
      Solana.SystemInstruction[
...
export const getSolanaInstructionParamKeyName = (
  key: SolanaInstructionParamKeys
) => {
  return (
    (
      {
      ...
      } as Partial<Record<SolanaInstructionParamKeys, string>>
    )[key] || key
  )
}
