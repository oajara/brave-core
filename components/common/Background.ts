...
export type SendResponseFunction<T> = (responsePayload: T) => void
export type HandlerFunction<T, U> = (
  payload: T,
  sender: any,
  sendResponse: SendResponseFunction<U>
) => void
export type NHandlerFunction<U> = (
  payload: void,
  sender: any,
  sendResponse: SendResponseFunction<U>
) => void
...
export function send<U = void, T = void>(
  messageType: string,
  payload?: T
): Promise<U> {
  // TODO: verify comms channel isn't closed prematurely first. If so, wait and try again.
  console.debug(`Sending data to brave extension for ${messageType}`, {
    messageType,
    payload
  })
  return new Promise(function (resolve) {
    chrome.runtime.sendMessage(
      braveExtensionId,
      { messageType, payload },
      function (responseData: U) {
        console.debug(
          `got response from brave extension for "${messageType}"`,
          responseData
        )
...
export function setListener<U = {}, T = void>(
  messageType: MessageName,
  handler: HandlerFunction<T, U>
): void {
  if (!messageHandlers) {
...
function onMessageExternal<T>(
  ...
  if (!isAllowedMessageSender(sender)) {
    console.warn(
      `Received external message from a sender who is not allowed to send messages to this background. Origin: ${sender.origin}, URL: ${sender.url}`
    )
    return
  }
  // validate
  if (!req.messageType) {
    console.warn(
      `Unknown external message sent to Background from url ${sender.url}. No messageType property specified.`
    )
    return
  }
  console.debug(
    `Background received external message from ${sender.origin}: ${req.messageType}.`,
    sender.url
  )
  const handler = messageHandlers.get(req.messageType)
  if (!handler) {
    console.error(
      `No handler for received external message '${req.messageType}' received from ${sender.origin}`
    )
    return false
  }
