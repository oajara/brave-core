...
declare namespace chrome.braveRewards {
  ...
  const isUnsupportedRegion: (
    callback: (unsupportedRegion: boolean) => void
  ) => {}
  ...
  const getRewardsParameters: (
    callback: (properties: RewardsExtension.RewardsParameters) => void
  ) => {}
  ...
  const getPublisherInfo: (
    ...
    callback: (
      ...
      properties: RewardsExtension.PublisherInfo
    ) => void
  ) => {}
  const getPublisherPanelInfo: (
    publisherKey: string,
    callback: (
      result: RewardsExtension.Result,
      properties: RewardsExtension.PublisherInfo
  ...
  const getPublisherInfoForTab: (
    tabId: number,
    callback: (publisher?: RewardsExtension.PublisherInfo) => void
  ) => void
  ...
  const savePublisherInfo: (
    ...
    favIconUrl: string,
    callback: (result: RewardsExtension.Result) => void
  ) => {}
  const tipSite: (
    tabId: number,
    publisherKey: string,
    entryPoint: RewardsExtension.TipDialogEntryPoint
  ) => {}
  const tipUser: (
    tabId: number,
    mediaType: string,
  ...
  const getBalanceReport: (
    month: number,
    year: number,
    callback: (properties: RewardsExtension.BalanceReport) => void
  ) => {}
  const onPublisherData: {
    addListener: (
      callback: (
        windowId: number,
        publisher: RewardsExtension.Publisher
      ) => void
    ) => void
  }
  const onPromotions: {
    addListener: (
      callback: (
        result: RewardsExtension.Result,
        promotions: RewardsExtension.Promotion[]
      ) => void
    ) => void
  }
  const onPromotionFinish: {
    addListener: (
      callback: (
        result: RewardsExtension.Result,
        promotion: RewardsExtension.Promotion
      ) => void
    ) => void
  }
  ...
  const fetchPromotions: (
    callback: (promotions: RewardsExtension.Promotion[]) => void
  ) => {}
  const claimPromotion: (
    promotionId: string,
    callback: (properties: RewardsExtension.Captcha) => void
  ) => {}
  const attestPromotion: (
    promotionId: string,
    solution: string,
    callback: (result: number, promotion?: RewardsExtension.Promotion) => void
  ) => {}
  ...
  const onPublisherListNormalized: {
    addListener: (
      callback: (properties: RewardsExtension.PublisherNormalized[]) => void
    ) => void
  }
  const onExcludedSitesChanged: {
    addListener: (
      callback: (properties: RewardsExtension.ExcludedSitesChanged) => void
    ) => void
  }
  const saveSetting: (key: string, value: string) => {}
  const getRecurringTips: (
    callback: (tips: RewardsExtension.RecurringTips) => void
  ) => {}
  ...
  const refreshPublisher: (
    publisherKey: string,
    callback: (status: number, publisherKey: string) => void
  ) => {}
  const getAllNotifications: (
    callback: (list: RewardsExtension.Notification[]) => void
  ) => {}
  const getInlineTippingPlatformEnabled: (
    key: string,
    callback: (enabled: boolean) => void
  ) => {}
  const fetchBalance: (
    callback: (balance: RewardsExtension.Balance) => void
  ) => {}
  ...
  const getExternalWallet: (
    callback: (wallet?: RewardsExtension.ExternalWallet) => void
  ) => {}
  ...
  const onDisconnectWallet: {
    addListener: (
      callback: (properties: { result: number; walletType: string }) => void
    ) => void
  }
...
declare namespace chrome.binance {
  ...
  const getDepositInfo: (
    symbol: string,
    tickerNetwork: string,
    callback: (depositAddress: string, depositTag: string) => void
  ) => {}
  const getCoinNetworks: (
    callback: (networks: Record<string, string>) => void
  ) => {}
...
declare namespace chrome.gemini {
  ...
  const getAccountBalances: (
    callback: (balances: Record<string, string>, authInvalid: boolean) => void
  ) => {}
  const getDepositInfo: (
    asset: string,
    callback: (depositAddress: string, depositTag: string) => void
  ...
  const getOrderQuote: (
    ...
    symbol: string,
    spend: string,
    callback: (quote: any, error: string) => void
  ) => {}
  const executeOrder: (
    symbol: string,
    side: string,
...
declare namespace chrome.ftx {
  ...
  const getChartData: (
    ...
    callback: (data: ChartData) => unknown
  ) => {}
  ...
  const getAccountBalances: (
    callback: (balances: Balances, authInvalid: boolean) => void
  ) => {}
  const getConvertQuote: (
    from: string,
    to: string,
    amount: string,
    callback: (quoteId: string) => void
  ) => {}
  const getConvertQuoteInfo: (
    quoteId: string,
    callback: (quote: QuoteInfo) => void
  ) => {}
  const executeConvertQuote: (
    quoteId: string,
    callback: (success: boolean) => void
  ) => {}
  const isSupported: (callback: (supported: boolean) => void) => {}
