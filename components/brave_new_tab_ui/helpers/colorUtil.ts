...
export class ColorData {
  static #luminanceCache = new Map([
    [0 /* black */, 0.0],
    [255255255 /* white */, 1.0]
  ])

  static clearCacheForTesting() {
    ColorData.#luminanceCache = new Map([
      [0 /* black */, 0.0],
      [255255255 /* white */, 1.0]
    ])
  }
  ...
  constructor(value: string) {
    ...
    if (rgb) {
      ...
      ;[this.r, this.g, this.b] = [rgb[1], rgb[2], rgb[3]].map((v) =>
        parseInt(v, 10)
      )
      return
    }
...
export function getContrastRatio(a: ColorData, b: ColorData): number {
  const [lighter, darker] =
    a.getRelativeLuminance() > b.getRelativeLuminance() ? [a, b] : [b, a]
  return (
    (lighter.getRelativeLuminance() + 0.05) /
    (darker.getRelativeLuminance() + 0.05)
  )
}
 ...
 */
export default function isReadableOnBackground(
  background: NewTab.BackgroundWallpaper | undefined
) {
  ...
  return (
    isReadable(backgroundData, new ColorData('#fff')) &&
