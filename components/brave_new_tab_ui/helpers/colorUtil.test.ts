...
describe('ColorData', () => {
  ...
  describe('getRelativeLuminance()', () => {
    ...
    it('returns 0.0 for black', () => {
      expect(
        new ColorUtil.ColorData('rgb(0, 0, 0)').getRelativeLuminance()
      ).toBe(0)
    })
    it('returns 1.0 for white', () => {
      expect(
        new ColorUtil.ColorData('rgb(255, 255, 255)').getRelativeLuminance()
      ).toBe(1)
    })
    it('returns 0.2126 for red', () => {
      expect(
        new ColorUtil.ColorData('rgb(255, 0, 0)').getRelativeLuminance()
      ).toBe(0.2126)
    })
    it('returns 0.7152 for green', () => {
      expect(
        new ColorUtil.ColorData('rgb(0, 255, 0)').getRelativeLuminance()
      ).toBe(0.7152)
    })
    it('returns 0.0722 for blue', () => {
      expect(
        new ColorUtil.ColorData('rgb(0, 0, 255)').getRelativeLuminance()
      ).toBe(0.0722)
    })
  })
  ...
  describe('luminanceCache', () => {
    beforeEach(() => {
      ColorUtil.ColorData.clearCacheForTesting()
    })

    it('has L for rgb(255, 255, 255) in the beginning', () => {
      expect(
        new ColorUtil.ColorData('rgb(255, 255, 255)').hasCachedLuminance()
      ).toBeTruthy()
    })
    it('has L for #ffffff in the beginning', () => {
      expect(
        new ColorUtil.ColorData('#ffffff').hasCachedLuminance()
      ).toBeTruthy()
    })
    it('has L for rgb(0, 0, 0) in the beginning', () => {
      expect(
        new ColorUtil.ColorData('rgb(0, 0, 0)').hasCachedLuminance()
      ).toBeTruthy()
    })
    it('has L for #000 in the beginning', () => {
      expect(new ColorUtil.ColorData('#000').hasCachedLuminance()).toBeTruthy()
    })
...
describe('getContrastRatio()', () => {
  ...
  it('returns 1.0 for same color', () => {
    expect(
      ColorUtil.getContrastRatio(
        new ColorUtil.ColorData('rgb(0, 0, 0)'),
        new ColorUtil.ColorData('#000')
      )
    ).toBe(1)
  })
  ...
  it('returns 2.91 for red and green', () => {
    expect(
      ColorUtil.getContrastRatio(
        new ColorUtil.ColorData('#f00'),
        new ColorUtil.ColorData('#0f0')
      )
    ).toBe(2.9139375476009137)
  })
  ...
  it('returns 6.26 for green and blue', () => {
    expect(
      ColorUtil.getContrastRatio(
        new ColorUtil.ColorData('#0f0'),
        new ColorUtil.ColorData('#00f')
      )
    ).toBe(6.261865793780687)
  })
...
describe('isReadable()', () => {
  describe('When background is black, stats are visible', () => {
    ...
    it('returns true for tracker stat color', () => {
      expect(
        ColorUtil.isReadable(
          background,
          new ColorUtil.ColorData('#FB542B' /* --interactive2 */)
        )
      ).toBeTruthy()
    })
    it('returns true for bandwidth saved stat color', () => {
      expect(
        ColorUtil.isReadable(
          background,
          new ColorUtil.ColorData('#A0A5EB' /* --interactive9 */)
        )
      ).toBeTruthy()
    })
    it('returns true for time saved stat color', () => {
      expect(
        ColorUtil.isReadable(background, new ColorUtil.ColorData('#FFFFFF'))
      ).toBeTruthy()
    })
  })
  ...
  describe('When background is #F0CB44, stats are not visible', () => {
    ...
    it('returns false for bandwidth saved stat color', () => {
      expect(
        ColorUtil.isReadable(
          background,
          new ColorUtil.ColorData('#A0A5EB' /* --interactive9 */)
  ...
  describe('When background is #2197F9 stats are not visible', () => {
    ...
    it('returns false for tracker stat color', () => {
      expect(
        ColorUtil.isReadable(
          background,
          new ColorUtil.ColorData('#FB542B' /* --interactive2 */)
