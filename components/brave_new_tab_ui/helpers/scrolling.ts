...
export const getScrollableParents = (
  ...
  while (
    element &&
    (!options?.limit || scrollableElements.length < options.limit)
  ) {
...
export const getScrollableParent = (element: Element | null | undefined) =>
  getScrollableParents(element, { limit: 1 })[0]

export const useParentScrolled = (
...
export const useMaintainScrollPosition = (
  ...
  React.useEffect(() => {
    ...
    const scrollPosition: { top: number; left: number } | null = JSON.parse(
