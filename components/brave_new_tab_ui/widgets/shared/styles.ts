...
function getTextStyle(p: StyleProps) {
  return [
    ...
    [
      '$fontSize',
      `font-size: ${
        (p.$fontSize && `${p.$fontSize}px`) ||
        (p.small ? '11px' : p.large ? '19px' : '13px')
      };`
    ],
...
export const Text = styled('p')<StyleProps>`
  ...
  font-size: ${(p) => {
    return (
      (p.$fontSize && `${p.$fontSize}px`) ||
      (p.small ? '11px' : p.large ? '19px' : '14px')
    )
  }};
...
export const ActionButton = styled('button')<StyleProps>`
  ...
  background: ${(p) =>
    p.light ? 'rgba(255, 255, 255, 0.21)' : p.theme.color.primaryBackground};
  border: 0;
...
export const FilterOption = styled(PlainButton)<StyleProps>`
  color: ${(p) =>
    p.isActive ? p.theme.color.primaryBackground : getColor('light')};
  font-weight: 600;
  text-transform: uppercase;
