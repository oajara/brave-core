...
function stateWithBalancesAndMarkets(
  state: FTXState,
  incomingMarketData: chrome.ftx.TokenPriceData[],
  ...
  if (marketData && balances) {
    for (const currency of Object.keys(balances)) {
      ...
      const balanceUSD = convertBalanceToUSD(
        marketData,
        currency,
        currencyAmount
      )
...
reducer.on(Actions.dataUpdated, (state, payload) => {
  return stateWithBalancesAndMarkets(
    state,
    payload.marketData,
    payload.balances
  )
