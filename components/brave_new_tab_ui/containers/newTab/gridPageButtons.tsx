...
function GridPageButton(props: GridPageButtonProps) {
  const handleClick = () => {
    const element = props.pageContainerRef.current?.children[
      props.page
    ] as HTMLElement
    if (!element) return
    ...
    props.pageContainerRef.current?.scrollTo({
      left: element.offsetLeft,
      behavior: 'smooth'
    })
  }

  return <StyledButton onClick={handleClick}>{PageIndicator}</StyledButton>
}
...
export const GridPageButtons = (props: {
  numPages: number
  pageContainerRef: React.MutableRefObject<HTMLDivElement | undefined>
}) => {
