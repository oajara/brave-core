 ...
 */
function isPublisherContentAllowed(
  publisher: Publisher,
  channels: Channels
): boolean {
...
export function getPublisherChannels(publisher: Publisher) {
  ...
  return newTabData.featureFlagBraveNewsV2Enabled
...
function ListItem(props: ListItemProps) {
  ...
  React.useEffect(() => {
    if (rowRoot.current) {
      const marginBottom = parseFloat(
        getComputedStyle(rowRoot.current).marginBottom || '0'
      )

      setSize &&
        setSize(
          props.index,
          rowRoot.current.getBoundingClientRect().height + marginBottom
        )
    }
  ...
  const isChecked =
    publisher && isPublisherContentAllowed(publisher, props.channels)
  ...
  return (
    <s.PublisherListItem style={props.style}>
      ...
      >
        ...
        <Toggle checked={isChecked} onChange={onChange} size="large" />
      </SettingsRow>
    </s.PublisherListItem>
  )
}
...
const channelsPromise = getBraveNewsController()
  .getChannels()
  .then((r) => r.channels as Channels)
