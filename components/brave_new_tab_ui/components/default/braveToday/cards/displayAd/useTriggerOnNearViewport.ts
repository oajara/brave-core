...
export default function useTriggerOnNearViewport(
  handlerRef: React.MutableRefObject<Function>
) {
  ...
  React.useEffect(() => {
    ...
    contentTriggerObserver.current = new IntersectionObserver(
      async (entries) => {
  ...
  React.useEffect(() => {
    ...
    console.debug(
      'Brave News: ad fetch trigger observer connected',
      debugInstanceId
    )
    observer.observe(contentTrigger.current)
    return () => {
      console.debug(
        'Brave News: ad fetch trigger observer disconnected',
        debugInstanceId
      )
      observer.disconnect()
    }
