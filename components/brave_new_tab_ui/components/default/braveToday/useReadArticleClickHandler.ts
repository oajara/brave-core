...
import {
  ReadFeedItemPayload,
  VisitDisplayAdPayload
} from '../../../actions/today_actions'
import { OnReadFeedItem, OnVisitDisplayAd } from './'

export default function useReadArticleClickHandler(
  action: OnReadFeedItem,
  payloadData: ReadFeedItemPayload
) {
...
export function useVisitDisplayAdClickHandler(
  action: OnVisitDisplayAd,
  payloadData?: VisitDisplayAdPayload
) {
  return React.useCallback(
    (e: React.MouseEvent) => {
      e.preventDefault()
