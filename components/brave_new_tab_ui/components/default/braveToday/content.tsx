...
export default function BraveTodayContent(props: Props) {
  ...
  const setScrollTriggerRef = React.useCallback(
    (element) => {
      if (!element) {
        return
      }
      ...
      const endOfCurrentArticlesListObserver = new IntersectionObserver(
        (entries) => {
          console.debug(
            'Brave Today content Intersection Observer triggered',
            entries
          )
          ...
          if (hasReachedPaginationPoint) {
            console.debug(
              'Brave Today content Intersection Observer determined need new page.'
            )
  ...
  const interactionObserver = React.useMemo<IntersectionObserver>(() => {
    const observer = new IntersectionObserver((entries) => {
      ...
      console.debug(
        'Brave News: Intersection Observer trigger show options, changing',
        isInteracting
      )
      ...
      if (
        isInteracting &&
        props.isPrompting &&
        !hasMarkedInteractionBegin.current
      ) {
  ...
  const intersectionObserver = React.useRef(
    new IntersectionObserver((entries) => {
      ...
      const counts = inView.map((i) =>
        Number(i.target.getAttribute(attributeNameCardCount))
      )
      counts.sort((a, b) => b - a)
  ...
  const registerCardCountTriggerElement = React.useCallback(
  ...
  if (!feed || !publishers) {
    console.error(
      'Brave Today: should have shown error or loading state, but ran in to an unintended code path.'
    )
    return null
  }

  const isOnlyDisplayingPeekingCard = !props.hasInteracted && props.isPrompting
  const displayedPageCount = Math.min(
    props.displayedPageCount,
    feed.pages.length
  )
  ...
  return (
    <>
      ...
      <div
        {...{ [attributeNameCardCount]: 1 }}
        ref={registerCardCountTriggerElement}
      />
      {!isOnlyDisplayingPeekingCard && (
        <>
          <>
            ...
            />
            <div
              {...{ [attributeNameCardCount]: introCount }}
              ref={registerCardCountTriggerElement}
            />
          </>
          {
            /* Infinitely repeating collections of content. */
            Array(displayedPageCount)
              .fill(undefined)
              .map((_: undefined, index: number) => {
                const shouldScrollToDisplayAd =
                  props.displayAdToScrollTo === index + 1
          ...
          <Customize
            onCustomizeBraveToday={() =>
              loadTimeData.getBoolean('featureFlagBraveNewsV2Enabled')
                ...
                : props.onCustomizeBraveToday()
            }
            show={showOptions}
          />
          <Refresh
            isFetching={props.isFetching}
            show={showOptions && (props.isUpdateAvailable || props.isFetching)}
