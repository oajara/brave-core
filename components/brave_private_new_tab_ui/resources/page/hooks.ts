...
export function useTorObserver() {
  ...
  React.useEffect(() => {
    getPageHandlerInstance().callbackRouter.onTorCircuitEstablished.addListener(
      setHasInit
    )
    getPageHandlerInstance().callbackRouter.onTorInitializing.addListener(
      (progress: string, message: string) => {
        setProgress(progress)
        setMessage(message)
      }
    )
    getPageHandlerInstance().callbackRouter.onTorCircuitGetStuck.addListener(
      setFailed
    )
  }, [])
...
export function useHasDisclaimerDismissed() {
  ...
  const [hasDisclaimerDismissed, setHasDisclaimerDismissed] = React.useState<
    boolean | null
  >(null)

  React.useEffect(() => {
    getPageHandlerInstance()
      .pageHandler.getDisclaimerDismissed()
