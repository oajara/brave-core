...
void SearchResultAdService::OnTriggerSearchResultAdViewedEvent(
  ...
  if (!success) {
    VLOG(1) << "Error during processing of search result ad viewed event for "
            << placement_id;
  }
}

void SearchResultAdService::TriggerSearchResultAdClickedEvent(
    ads::mojom::SearchResultAdPtr search_result_ad) {
  ads_service_->TriggerSearchResultAdEvent(
...
void SearchResultAdService::OnTriggerSearchResultAdClickedEvent(
    ...
    ads::mojom::SearchResultAdEventType ad_event_type) {
  if (!success) {
    VLOG(1) << "Error during processing of search result ad clicked event for "
            << placement_id;
  }
}
