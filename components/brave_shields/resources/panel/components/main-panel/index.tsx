...
function MainPanel() {
  ...
  const braveShieldsStatusText = splitStringForTag(
    siteBlockInfo?.isBraveShieldsEnabled
      ? getLocale('braveShieldsUp')
      : getLocale('braveShieldsDown')
  )
  const braveShieldsBrokenText = splitStringForTag(
    getLocale('braveShieldsBroken')
  )
  const braveShieldsNote = splitStringForTag(
    siteBlockInfo?.isBraveShieldsEnabled
      ...
      <i>
        ...
        >
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M15.334 8.969H6a.667.667 0 1 1 0-1.334h9.334a.667.667 0 0 1 0 1.334Zm.005-5.377H5.962c-.368 0-.629-.255-.629-.623s.299-.667.667-.667h9.334c.367 0 .666.299.666.667 0 .368-.292.623-.66.623ZM2 15.635c-1.102 0-2-.897-2-2s.898-2 2-2c1.103 0 2 .897 2 2s-.897 2-2 2Zm0-2.666a.667.667 0 1 0 .001 1.334.667.667 0 0 0 0-1.334Zm0-2.667c-1.102 0-2-.897-2-2s.898-2 2-2c1.103 0 2 .897 2 2s-.897 2-2 2Zm0-2.667a.667.667 0 1 0 .002 1.335A.667.667 0 0 0 2 7.635Zm.398-3.604a.669.669 0 0 1-.96.12L.244 3.17a.666.666 0 1 1 .846-1.03l.65.533L2.798 1.24a.668.668 0 0 1 1.073.791l-1.472 2ZM6 12.969h9.334a.667.667 0 0 1 0 1.333H6a.667.667 0 1 1 0-1.333Z"
          />
        </svg>
      </i>
  ...
  let totalCountElement = (
    <S.BlockCount title={siteBlockInfo?.totalBlockedResources.toString()}>
      {(siteBlockInfo?.totalBlockedResources ?? 0) > 99
        ? '99+'
        : siteBlockInfo?.totalBlockedResources}
    </S.BlockCount>
  )
  ...
  if (!siteBlockInfo?.isBraveShieldsEnabled) {
    ...
    advancedControlButtonElement = (
      <S.GlobalDefaultsButton type="button" onClick={onSettingsClick}>
    ...
    reportSiteOrFootnoteElement = (
      <S.ReportSiteBox>
        <S.ReportSiteAction>
          ...
          <Button isPrimary onClick={handleReportSite}>
  ...
  return (
    <S.Box>
      ...
      <S.StatusBox>
        <S.ControlBox>
          <S.ShieldsIcon
            isActive={siteBlockInfo?.isBraveShieldsEnabled ?? false}
          >
            <svg
              width="24"
              height="28"
          ...
          <S.StatusText>
            ...
            {braveShieldsStatusText.afterTag} {siteBlockInfo?.host}
          </S.StatusText>
          <S.StatusToggle>
            <Toggle
              brand="shields"
        ...
        {siteBlockInfo?.isBraveShieldsManaged && (
          <S.StatusFootnoteBox>{managedFootnoteElement}</S.StatusFootnoteBox>
        )}
      </S.StatusBox>
      {advancedControlButtonElement}
      {isExpanded && siteBlockInfo?.isBraveShieldsEnabled && (
