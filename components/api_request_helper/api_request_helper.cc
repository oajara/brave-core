...
bool APIRequestResult::operator==(const APIRequestResult& other) const {
  auto tied = [](auto& v) {
    return std::tie(v.final_url_, v.error_code_, v.response_code_, v.body_,
                    v.value_body_, v.headers_);
  };
...
void APIRequestHelper::OnResponse(
  ...
  data_decoder::DataDecoder::ParseJsonIsolated(
      raw_body, base::BindOnce(&OnSanitize, response_code, std::move(headers),
